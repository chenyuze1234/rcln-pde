"""
Generate CORRECT data with proper amplitude
Fix: Ensure data has proper magnitude
"""
import numpy as np
import torch
...
dt = 0.20
n_steps = 5
n_warmup = 200  # Reduced from 300 for speed

def pseudo_spectral_step(u_hat, kx, ky, kz, k2, nu, dt):
    """One step of pseudo-spectral NS"""
...
def generate_field(seed):
    """Generate turbulent IC with proper amplitude"""
    ...
    # FIX: Ensure amplitude is not too small
    ...
    # FIX: Normalize to have energy = 1
    u = fft.irfftn(u_hat[0], s=(N, N, N))
    v = fft.irfftn(u_hat[1], s=(N, N, N))
    w = fft.irfftn(u_hat[2], s=(N, N, N))
...
def main():
    ...
    print(f"Generating CORRECT data v4...")
    ...
    with h5py.File('data/experiment_v4_correct.h5', 'w') as f:
        ...
        print(f"  std:  {std:.6f}")
        print(f"  min:  {train_inputs.min():.6f}")
        print(f"  max:  {train_inputs.max():.6f}")
    
    print(f"\nSaved: data/experiment_v4_correct.h5")
    ...
    with h5py.File('data/experiment_v4_correct.h5', 'r') as f:
        inp = f['train/input'][:10]
        tgt = f['train/target'][:10]
        
        print(f"\nData range: [{inp.min():.4f}, {inp.max():.4f}]")
        print(f"Data std:   {inp.std():.4f}")
        ...
        if 0.88 <= mean_corr <= 0.97 and inp.std() > 0.5:
            print("\nGOOD - Data looks correct!")
        else:
            if inp.std() < 0.1:
                print("\nERROR: Data std too small!")
            if mean_corr > 0.97:
                print("\nWARNING: Correlation too high")
